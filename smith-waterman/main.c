...
int main(int argc, char *const argv[]) {
  ...
  while (kseq_read(read_seq) >= 0) {
    ...
    while (kseq_read(ref_seq) >= 0) {
      ...
      if (!result) {
        ...
        if (result->flag == 2)
          fprintf(stderr,
                  "Warning: The alignment of the following sequences may miss "
                  "a small part.\nref_seq: %s\nread_seq: %s\n\n",
                  ref_seq->name.s, read_seq->name.s);
        if (sam)
        ...
        fprintf(stderr,
                "There is no identical residue between the following reference "
                "and read seqeunces.\nref_name: %s\nread_name: %s\n\n",
                ref_seq->name.s, read_seq->name.s);
        continue;
      }
    ...
    long current_mem = get_memory_usage();
  ...
  fprintf(stderr, "  Initial: %ld MB\n", mem_start);
  fprintf(stderr, "  Peak:    %ld MB\n", mem_peak);
  fprintf(stderr, "  Delta:   %ld MB\n", mem_peak - mem_start);
